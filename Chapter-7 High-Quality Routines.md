...
什么是高质量的子程序？回答这个问题最简答的办法是，来看看什么东西不是高质量的子程序。这里有一个低质量子程序的例子，书中提示至少可以从中发现 10 处不同的问题。
...
“创建一个子程序可以有很多合理的原因，但完成它的方式却有对错之分。” Steve 在上大学的时候曾认为创建子程序的主要原因不过是为了避免重复代码，教材上也那样写。…… 俺也一样啊。7.1 节给出了一些更好的解释。
...
- 支持子类化。（注：咦，这是意思？）
- 隐藏顺序，把处理数据的顺序隐藏起来。（注：即将相关操作“封装”起来，如 `popStack` 包括弹栈和减少计数值两个操作）
- 隐藏指针操作。指针操作的可读性往往都很差，且容易出错，必要时可考虑将其隔离起来。
- 提高可移植性。即将使用子程序将不可移植的部分隔离起来，从而明确识别和隔离未来的移植工作。
...
编写有效的子程序时，一个最大的心理障碍就是：不情愿为一个简单的目的而编写一个简单的子程序。比如一个两三行的子程序看起来有点大材小用。有时，情况事情看起来发生了一些变化。比如下面这一行代码：
...
- 功能的内聚性。最强也是最好的一种内聚性，即让子程序仅执行一项操作，如 `sin()，EraseFile()` 等。这种方式评价内聚性的前提是子程序执行的操作和名字相符合。除此之外，一些其他的内聚性通常不太理想。
...
这些规则都不是神圣不可侵犯的，需要理解其想法并根据实际情况运用。幸运的是，编写功能上的内聚性的子程序几乎都是可能的，因此把注意力集中在功能内聚性上，从而得到最大的收获（注：7.2节列举了很多精彩的例子以阐述各种内聚性的含义）。
...
7.4 节列举了很多子程序长度与错误量关系的研究（注：其中的部分结论貌似相互矛盾）。Steve老师的总结是：与其对子程序的长度强加限制，不如让下面的因素来决定子程序的长度。这些因素包括：子程序的内聚性、嵌套的层次、变量的数量、决策点的数量、解释子程序用以所需的注释数量，以及其他一些跟复杂度相关的考虑事项等。
